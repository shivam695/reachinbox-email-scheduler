...
const worker = new Worker<EmailJobData>(
  ...
  async (job) => {
    ...
    // Use the campaign's chosen hourly limit; fall back to the global env default only if missing.
    const hourlyLimit =
      email.campaign?.hourlyLimit || Number(process.env.MAX_EMAILS_PER_HOUR) || 200;
